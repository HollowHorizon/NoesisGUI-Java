...
void MTLRenderContext::Init(void* window, uint32_t& samples, bool vsync, bool sRGB)
{
    ...
    // For iOS this is not necessary as we are using CADisplayLink
    // For macOS (we are not using CVDisplayLink) we need the semaphore to limit the number of
    // queued frames (nextDrawable should limit this but on Apple M1 sometimes it doesn't)
    mSemaphore = dispatch_semaphore_create(MAX_QUEUED_FRAMES);
    ...
    MTL_NAME(mCommandQueue, @"Noesis_CommandQueue");
    ...
    MTLFactory::PreCreatePipeline(mRenderer, mMetalLayer.pixelFormat, MTLPixelFormatStencil8, mSamples);
    MTLFactory::UnloadLibrary(mRenderer);
}
...
void MTLRenderContext::BeginRender()
{
    ...
    MTL_NAME(mCommandBuffer, @"Noesis_Frame");
...
void MTLRenderContext::Resize()
{
    ...
    if (mSamples > 1)
    {
        ...
        MTL_NAME(colorAA, @"Noesis_FB_ColorAA");
    ...
    MTL_NAME(stencil, @"Noesis_FB_Stencil");
...
void MTLRenderContext::SetDefaultRenderTarget(uint32_t, uint32_t, bool doClearColor)
{
    ...
    MTL_NAME(mDrawable.texture, @"Noesis_FB_Color");
    ...
    MTL_NAME(mCommandEncoder, @"[R] Main Screen");

    MTLFactory::SetOnScreenEncoder(mRenderer, mCommandEncoder, mMetalLayer.pixelFormat,
        MTLPixelFormatStencil8, mSamples);
}

////////////////////////////////////////////////////////////////////////////////////////////////////
Noesis::Ptr<NoesisApp::Image> MTLRenderContext::CaptureRenderTarget(RenderTarget* surface) const
{
