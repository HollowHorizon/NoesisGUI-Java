...
MTLRenderDevice::MTLRenderDevice(id<MTLDevice> device, bool sRGB): mDevice(device), mCommands(0),
    mCommandEncoder(0), mBlitEncoder(0)
{
    BindAPI();
    FillCaps(sRGB);
    CreateBuffers();
    ...
    uint32_t height, uint32_t numLevels, TextureFormat::Enum format, const void** data)
{
    ...
    if (data != 0)
    {
        id<MTLBlitCommandEncoder> encoder = [mCommands blitCommandEncoder];

        for (uint32_t i = 0; i < numLevels; i++)
        {
            uint32_t mipWidth = Max(width >> i, 1U);
            uint32_t mipHeight = Max(height >> i, 1U);
            uint32_t pitch = format == TextureFormat::R8 ? mipWidth : mipWidth * 4;
            uint32_t size = mipHeight * pitch;

            id<MTLBuffer> source = [mDevice newBufferWithLength:size options:0];
            memcpy([source contents], data[i], size);

            [encoder copyFromBuffer:source sourceOffset:0 sourceBytesPerRow:pitch
                sourceBytesPerImage:0 sourceSize:{ mipWidth, mipHeight, 1}
                toTexture:texture destinationSlice:0 destinationLevel:i
...
{
    if (mBlitEncoder == nullptr)
    {
        mBlitEncoder = [mCommands blitCommandEncoder];
    }

    MTLTexture* texture = (MTLTexture*)texture_;
    uint32_t pitch = texture->object.pixelFormat == MTLPixelFormatR8Unorm ? width : width * 4;
    void* ptr = MapBuffer(mTexUpload, Align(height * pitch, 4));
    memcpy(ptr, data, height * pitch);

    [mBlitEncoder copyFromBuffer:mTexUpload.currentPage->object sourceOffset:mTexUpload.drawPos
        sourceBytesPerRow:pitch sourceBytesPerImage:0 sourceSize:{ width, height, 1}
        toTexture:texture->object destinationSlice:0 destinationLevel:level
        destinationOrigin: { x, y, 0}];
}

////////////////////////////////////////////////////////////////////////////////////////////////////
void MTLRenderDevice::EndUpdatingTextures(Texture**, uint32_t)
{
    NS_ASSERT(mBlitEncoder != 0);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
MTLRenderDevice::Page* MTLRenderDevice::AllocatePage(PageAllocator& allocator)
{
...
void MTLRenderDevice::DestroyBuffer(DynamicBuffer& buffer)
{
    ...
    while (block)
    {
        ...
        {
            MTL_RELEASE(block->pages[i].object);
        }
